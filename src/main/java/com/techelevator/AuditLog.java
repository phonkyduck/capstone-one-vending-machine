...
public class AuditLog {
    ...
    public static void giveChangeLog(String giveChangeMessage, BigDecimal startingBalance, BigDecimal customerBalance){
        File auditLog = new File("src/main/java/com/techelevator/SalesLog.txt");
        giveChangeMessage = dateTimeFormat.format(now) + " GIVE CHANGE: $" + String.format("%.2f", startingBalance) + " $" + String.format("%.2f", customerBalance);
        try(PrintWriter purchaseAuditor = new PrintWriter(new FileOutputStream(auditLog, true))){
            purchaseAuditor.println(giveChangeMessage);
